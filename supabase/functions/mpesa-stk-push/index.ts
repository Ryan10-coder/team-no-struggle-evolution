...
serve(async (req) => {
  ...
  try {
    ...
    // Only the admin portal can initiate an STK push.
    if (action !== "stk_push") {
    ...
    const hasPermission = await verifyRole(
      userSupabase,
      user.id,
      ["admin", "treasurer", "secretary", "area_coordinator", "general_coordinator"],
    );

    if (!hasPermission) {
      return jsonResponse(
        { success: false, error: "Insufficient permissions. Admin or Treasurer role required." },
        403,
      );
    }

    return await handleSTKPush(body, createClient(supabaseUrl, supabaseServiceKey));
  } catch (error) {
...
async function handleSTKPush(data: any, supabase: any) {
  ...
  if (!memberId || amount === undefined || !phoneNumber) {
    return jsonResponse(
      { success: false, error: "Member, amount and phone number are required." },
      400,
    );
  }
  ...
  if (!Number.isInteger(numericAmount) || numericAmount <= 0) {
    return jsonResponse(
      { success: false, error: "Amount must be a positive whole number in KES." },
      400,
    );
  }
  ...
  if (!msisdn) {
    return jsonResponse(
      { success: false, error: "Invalid Safaricom phone number. Use 0712345678 or 254712345678." },
      400,
    );
  }
  ...
  const environment = (Deno.env.get("MPESA_ENVIRONMENT") || "production").toLowerCase();

  if (!consumerKey || !consumerSecret || !passkey || !shortcode) {
    return jsonResponse({
      success: false,
      error: "M-PESA credentials are not fully configured. Set MPESA_CONSUMER_KEY, MPESA_CONSUMER_SECRET, MPESA_PASSKEY and MPESA_SHORTCODE.",
    }, 500);
  }
  ...
  try {
    ...
    if (!tokenResponse.ok || !tokenData.access_token) {
      ...
      return jsonResponse({
        success: false,
        error: tokenData.errorMessage || tokenData.error_description ||
    ...
    // This endpoint is intentionally separate from the STK endpoint.
    // Safaricom production callback URLs must be HTTPS and should not contain
    // keywords such as "mpesa" or "safaricom".
    ...
    if (paymentError) {
      ...
      return jsonResponse({
        success: false,
        error: "STK Push was accepted by M-PESA, but the pending payment could not be saved. Check the M-PESA transaction before retrying.",
    ...
    return jsonResponse({
      success: true,
      message: "STK Push sent successfully. Check the customer's phone for the M-PESA prompt.",
      data: {
